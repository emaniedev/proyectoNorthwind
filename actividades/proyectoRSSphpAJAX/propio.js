var apiURL="http://daw24.000webhostapp.com/rss.php?";
/**
 * llamadaAJAXGet
...
function recursosRSS(){
    llamadaAJAXGet("accion=recursosrss", "json", function(response){
        ...
        $.each(response, function (index, rss) { 
             var nueva="<tr><td>"+rss.id+"</td><td>"+rss.titulo+"</td><td><a href='"+rss.url+"'>"+rss.url+"</a></td><td><button class='btn btn-sm btn-block btn-info' onclick='cargar("+rss.id+");'>Info</button><button class='btn btn-sm btn-block btn-warning' onclick='borrar("+rss.id+");'>Eliminar</button></td></tr>"
             todas+=nueva;
        });
...
function cargar(id){
    llamadaAJAXGet("accion=cargar&id="+id, "json", function (response){
        ...
        $("#RSS").removeClass("in active");
        $("#Ver").addClass("in").addClass("active");
